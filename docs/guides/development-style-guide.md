...
# Digital Federation CRM UI Style Guide for LLMs
...
<x-dynamic-table 
    ...
    :headers="[
        'code' => __('entity.code'),
        'name' => __('entity.name'),
        'location' => __('entity.location'),
        'country.name' => __('entity.country'),
    ]" 
    :enable-actions="true"
    :enable-search="false">

    <!-- Custom columns -->
    <x-slot name="country.name">
        @php
            $country = $item->country;
        @endphp
        @if($country)
            <img src="{{ asset('img/flags/' . strtolower($country->code) . '.svg') }}" 
                 alt="{{ $country->name }}" 
...
<x-layout>
    <div class="previous-layout-classes">
        ...
        <!-- Filters -->
        <x-filter-form :route="'module.index'">
            <!-- Filter fields -->
        </x-filter-form>

        <!-- Table -->
        <x-dynamic-table :items="$items" :route="'module'" :headers="$headers" />
    </div>
</x-layout>
```

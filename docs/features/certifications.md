...
The `CertificationAttributionWizard` is a comprehensive Livewire component that manages the entire certification workflow:
...
GET  /entity/certification-attributed - List attributed certifications
...
### Running Migrations
```bash
# Add pricing fields to certifications
php artisan migrate --path=database/migrations/2025_07_04_124733_add_pricing_fields_to_certifications_table.php

# Drop slot-related tables
php artisan migrate --path=database/migrations/2025_07_04_130032_drop_certification_slot_tables.php
```

### Data Migration Considerations
1. **Existing Slots**: Export data before migration if needed
2. **Pricing**: Set appropriate prices for each certification
3. **Configuration**: Configure `requester_model` and `allow_entity_group_request`
4. **Validation**: Set `requires_admin_validation` for certifications needing approval

### Code Cleanup
Remove the following files after migration:
- Models: CertificationSlot, CertificationSlotPrices, CertificationSlotType
- Controllers: All CertificationSlotController variants
- Actions: All slot-related actions
- Views: All slot-related views
- Routes: All slot-related routes

...
3.  **Admin Validation**: A CMAS admin panel for overseeing license requests, reviewing documents, and approving or rejecting applications.
...
-   **Instructor Management (`DivingInstructorsController`)**: A separate system allows entities to manage their roster of associated instructors, independent of license requests.
-   **Admin Validation (`DivingLicenseValidationController`)**: Provides a dedicated interface for CMAS admins to review and process license applications.
...
-   **Individual Routes (15 endpoints)**: e.g., `/individual/diving-certifications`, `/individual/technical-director-invitations/{id}/accept`
-   **Admin Routes (10 endpoints)**: e.g., `/admin/diving-license-validation`, `/admin/diving-professional-certifications/{id}/approve`
...
3.  **Individual: Accept Invitation**
    ...
    *   Navigate to "Technical Director Invitations".
    *   Accept the pending invitation.

4.  **Admin: Validate License**
    *   Log in as a CMAS Admin.
...
-   **Database**: Check the `diving_professional_certifications` and `diving_technical_director_invitations` tables for correct data.

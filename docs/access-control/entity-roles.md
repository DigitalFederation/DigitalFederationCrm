...
> This platform is a generic federation-management system. The diving/CMAS naming in some
...
The `Entity` namespace has four roles that grant specific permissions to entities (e.g., clubs, schools, companies). After the `2025_10_01_172644_simplify_role_system` migration the entity roles are:
...
-   `entity-cmas`: For entities authorized to handle international (CMAS) certifications. Grants `access orders`. (Formerly `entity-company`.)

> **Note:** The pre-migration entity roles `entity-sport-admin`, `entity-diving-admin`, `entity-scientific-admin`, and `entity-company` no longer exist. The migration renamed `entity-sport-admin` → `entity-sport`, `entity-diving-admin` → `entity-diving-services`, and `entity-company` → `entity-cmas`, and deleted `entity-scientific-admin`.
...
> A separate `SyncEntityUserRolesAction` does sync the base `entity-admin` role (and any roles mapped to an administered entity's active licenses via the `license_roles` pivot) for users who administer an active entity. The `entity-cmas` role is assigned manually.
...
-   **International Certifications**: The `entity-cmas` role is checked when an entity attempts to purchase international certifications. This role is a prerequisite for these actions.
...
-   **Policy Enforcement**: The `DivingCoursePolicy` explicitly checks for the `entity-admin` role (and the `admin` role) to authorize actions like creating, updating, and deleting diving courses.

...
description: Critical business rules for local vs modalidade federation membership
---
...
4. [Rule 2: License Activation - Modalidade Federation](#rule-2-license-activation---modalidade-federation)
...
// Modalidade Federations (Sport Associations)
...
> **When an individual becomes a member of an entity (club), they are ONLY synced to the LOCAL federations (territorial associations) that the entity belongs to. They are NEVER automatically synced to modalidade federations.**
...
```php
// WRONG - This would sync to ALL federations including modalidade
...
## Rule 2: License Activation - Modalidade Federation

### Business Rule

> **When a sport license is activated for an individual, THAT is the trigger for the individual to become a member of ALL modalidade federations linked to that license.**

### Rationale

- Sport licenses can be tied to MULTIPLE federations via the `federation_licenses` pivot table
- An individual earns modalidade membership by having an active license in that sport
- Only federations where `is_local = false` (modalidade federations) trigger membership sync
...
```php
// When activating an individual license, sync to ALL modalidade federations
if ($individual && $license->license) {
    $modalidadeFederations = $license->license->federations()
        ->where('is_local', false)  // Only modalidade federations
        ->get();
...
Get ALL federations linked to license (via federation_licenses pivot)
         ↓
Filter only modalidade federations (is_local = false)
         ↓
Sync individual to EACH modalidade federation
         ↓
Individual is now a member of those Modalidade Federations
```
...
- Unlike entity-join sync (LOCAL only) or license sync (MODALIDADE only), this syncs to ANY federation specified in the affiliation
...
| `is_local` | boolean | `true` = Territorial Association, `false` = Modalidade or Main |
...
| `src/Domain/Licenses/Actions/ActivateLicenseAttributedAction.php` | Activates license AND syncs to modalidade federation |
...
- Primary Federation (Main) - skipped (not modalidade)
- Example Local Region (Local, `is_local = true`) - skipped (not modalidade)
- Example Sport Federation (Modalidade, `is_local = false`) - **SYNCED**
...
| Migration | Purpose |
|-----------|---------|
| `2026_01_03_184938_fix_is_local_for_modalidade_federations.php` | Fixed `is_local = 0` for international diving and other modalidade federations |
| `2026_01_03_185343_add_requires_local_federation_affiliation_to_evt_competitions.php` | Added event local federation requirement |
...
| Date | Change | Author |
|------|--------|--------|
| 2026-01-03 | Initial documentation of federation membership rules | Claude |
| 2026-01-03 | Fixed is_local for modalidade federations (international diving, etc.) | Claude |
| 2026-01-03 | Added license activation -> federation sync | Claude |
| 2026-01-03 | Added event local federation requirement | Claude |
| 2026-01-03 | Fixed license activation to sync ALL modalidade federations (many-to-many via federation_licenses) | Claude |
| 2026-01-04 | Added Rule 4: Main Federation Cannot Be Removed - prevents individuals from disassociating from the main federation | Claude |
| 2026-01-08 | Added Rule 5: Affiliation Activation - Direct Federation Membership - syncs individual to federation when affiliation is activated via subscription | Claude |

    ...
    public function __invoke(string $type, Entity $entity)
    {
        ...
        $licenses = Cache::remember($licensesCacheKey, 5, function () use ($type, $entity) {
            ...
            $federationIds = $entity->federations()
                ->where('entity_federation.active', true)
                ->pluck('federation_id');
            ...
            return License::query()
                ...
                ->whereDoesntHave('licensesAttributed', function (Builder $query) use ($entity) {
                    $query->where(['model_type' => Entity::class, 'model_id' => $entity->id]);
                })
                ->orderBy('name')
                ->get();
        });
...
Update the `loadLicenses()` method to use federation filtering:
...
Update the store method's license validation:

```php
// In the store() method, around line 58
$federationIds = $entity->federations()
    ->where('entity_federation.active', true)
    ->pluck('federation_id');
...
Create a new Livewire component for managing federation licenses:

```php
// app/Livewire/Admin/FederationLicenseManager.php
...
class FederationLicenseManager extends Component
{
    ...
    public function mount(Federation $federation)
    {
        ...
        $this->selectedLicenses = $federation->licenses()->pluck('id')->toArray();
        $this->loadAvailableLicenses();
    }

    public function loadAvailableLicenses()
    {
        $this->availableLicenses = License::query()
    ...
    {
        $this->federation->licenses()->sync($this->selectedLicenses);
    ...
    {
        $entityIds = $this->federation->entities()->pluck('id');
        foreach ($entityIds as $entityId) {
            Cache::forget("licenses_for_type_*_entity_{$entityId}");
        }
    }

...
diving-federation deployment (hence labels such as *CMAS* and *Associações Territoriais*),
...
> that manages CMAS content) is just an Association linked to the international committees — it
> is **not** the Admin.
...
> New federations are seeded with committee links by
> `database/migrations/2026_01_07_170731_seed_federation_committee_relationships.php`, which
> matches the international-content federation **by name** via
> `config('branding.international.name')` (env `INTERNATIONAL_FEDERATION_NAME`). If that name
> does not match the actual federation when the migration runs, the federation receives **no**
> international committee links and must be linked manually (admin UI or `syncWithoutDetaching`).

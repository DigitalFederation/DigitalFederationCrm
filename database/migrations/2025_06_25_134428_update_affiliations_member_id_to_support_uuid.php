...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        $memberIdColumn = DB::select("SHOW COLUMNS FROM affiliations WHERE Field = 'member_id'");

        if (! empty($memberIdColumn) && str_contains($memberIdColumn[0]->Type, 'varchar')) {
            // Column is already varchar (UUID), nothing to do
            return;
        }

        // Check if we need to migrate (in case of existing data)
        $hasData = DB::table('affiliations')
        ...
        if ($hasData) {
            ...
            throw new \Exception(
                'Cannot automatically migrate affiliations table: existing data found with integer member_id values. ' .
                'Please manually migrate the data or clear the affiliations table before running this migration.'
            );
        }

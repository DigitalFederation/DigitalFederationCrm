...
<div class="card">
    ...
    <div class="flex flex-col md:flex-row justify-between gap-x-2 items-end mb-4">
        <div class="w-full md:w-1/2">
            <label for="codeCmasDelegateFederation" class="block text-sm font-medium mb-1">{{ __('certifications.cmas_code') }}</label>
            <input type="text" name="technical_delegate[code_cmas_delegate_federation]" id="codeCmasDelegateFederation"
                   class="form-input w-full"
                   value="{{ old('technical_delegate.code_cmas_delegate_federation', $technical_delegate->code_cmas_delegate_federation ?? '') }}"
            >
            @error('technical_delegate.code_cmas_delegate_federation') <span

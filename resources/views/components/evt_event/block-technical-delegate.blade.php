            ...
            @if(isset($event->competitions->first()->technicalDelegates) && !empty($event->competitions->first()->technicalDelegates->first()))
                ...
                @if($event->competitions->first()->technicalDelegates->first()->federation)
                    <div>
                        ...
                        <p class="text-sm text-slate-600">{{ $event->competitions->first()->technicalDelegates->first()->federation?->code_cmas }}</p>
                    </div>
                @endif

                <div>
                    <p class="text-xs text-slate-400">{{ __('certifications.cmas_code') }}</p>
                    <p class="text-sm text-slate-600">{{ $event->competitions->first()->technicalDelegates->first()->code_cmas_delegate_federation }}</p>
                </div>
                <div>

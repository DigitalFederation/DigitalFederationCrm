    ...
    <div class="grow">
        <div class="p-6 space-y-6">
            ...
            <div class="sm:flex sm:items-center space-y-4 sm:space-y-0 sm:space-x-4 mt-5">
                ...
                <div class="sm:w-1/2">
                    <label class="block text-sm font-medium mb-1" for="is_shipping"> {{ __('Needs Shipping?') }}</label>
                    <select name="is_shipping" class="form-input w-full" id="is_shipping">
                        <option default value="0" @if(!$type->is_shipping) selected @endif>No</option>
                        <option value="1" @if($type->is_shipping) selected @endif>Yes</option>
                    </select>

                    @if($errors->has('is_shipping'))
                        <div class="text-xs mt-1 text-rose-500 h-2">
                            {{ $errors->first('is_shipping') }}
                        </div>
                    @endif
                </div>

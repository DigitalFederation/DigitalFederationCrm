<div class="card w-full overflow-hidden">
            ...
            <div>
                ...
                <p class="text-slate-500 hover:text-slate-700 hover:underline">
                    ...
                    @if(!empty($certification->individual))
                        ({{ $certification->individual?->code_cmas }})
                    @endif
                </p>
            </div>
    ...
    @php
        ...
        $isCmas = auth()->user()->group()->first()->code == 'ADMIN';
    ...
    @if($isPendingCertification && ($isFederation || $isCmas))

        <div class="md:flex  flex-col gap-y-4  md:gap-4 mt-4 justify-between">

            @if(($isFederation && !$is_local) || $isCmas)
    ...
    @if($isActiveCertification && ($isFederation || $isCmas))
        <form
    ...
    @if($canUnsuspend && ($isFederation || $isCmas))
        <form

    ...
    @if($individual)
        <div class="bg-white rounded-lg border border-slate-200 p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                @if($individual->code_cmas)
                    <div>
                        <label class="block text-sm font-medium text-slate-700">{{ __('main.CMAS Code') }}</label>
                        <div class="mt-1 text-slate-900">{{ $individual->code_cmas }}</div>
                    </div>
                @endif
            </div>
        </div>

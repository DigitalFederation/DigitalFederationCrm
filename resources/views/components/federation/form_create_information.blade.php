...
<section>
    ...
    <div class="sm:flex sm:items-start space-y-4 sm:space-y-0 sm:space-x-4 mt-5">
        <div class="sm:w-1/3">
            <label class="block text-sm font-medium mb-1" for="code_cmas"> {{ __('main.CMAS Code' )}} <span class="text-rose-500">*</span></label>
            <input id="code_cmas"
                    class="form-input w-full {{ $errors->has('code_cmas') ? 'border-rose-300' : '' }}"
                    type="text" name="code_cmas"
                    value="{{ old('code_cmas', $federation->code_cmas ?? null) }}" required/>


            @if($errors->has('code_cmas'))
                <div class="text-xs mt-1 text-rose-500 h-2">
                    {{ $errors->first('code_cmas') }}
                </div>
            @endif
        </div>
    ...
    <div class="sm:flex sm:items-center space-y-4 sm:space-y-0 sm:space-x-4 mt-5">
        <div class="sm:w-5/5">
            ...
            <div class="flex information-box items-center w-full py-1 text-xs mt-1">
                ...
                <p class="text-xs">
                    Select in case the federation or organisation is regional, territorial, state or a federation with representation of diving, science or one or more underwater sports dependent on a main federation with representation from that country in the CMAS.
                </p>
            </div>
        </div>
    </div>

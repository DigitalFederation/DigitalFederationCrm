...
@php
    ...
    // Add invoices tab if in CMAS or Federation context and has payment documents
    if (($isCmasContext || $isFederationContext) && $payment_documents && $payment_documents->isNotEmpty()) {
        $tabsConfig['invoices'] = [
    ...
    // Add diving tabs if individual has diving data and in CMAS context
    if ($isCmasContext) {
    ...
    <x-ui.tabs :tabs="$tabsConfig" defaultTab="overview">
        ...
        <x-slot name="tab_overview">
            <div class="space-y-6">
                ...
                <!-- User Account Info (if CMAS context) -->
                @if ($showUserAccount && $isCmasContext && $individual->user)
        ...
        <x-slot name="tab_insurances">
            <div class="space-y-6">
                @if($allInsurances->isNotEmpty())
                    <x-individual.insurance-table :insurances="$allInsurances" :showActions="$isCmasContext" :context="$context" />
                @else
                    <div class="text-center py-12">
                        <svg class="mx-auto h-12 w-12 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z"></path>
        ...
        <x-slot name="tab_membership_affiliations">
            <div class="space-y-6">
                @if($activeAffiliations->isNotEmpty())
                    <x-individual.affiliation-table :affiliations="$activeAffiliations" :showActions="$isCmasContext" />
                @else
                    <div class="text-center py-12">
                        <svg class="mx-auto h-12 w-12 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        @if ($showDocuments)
            <x-slot name="tab_documents">
                <div class="space-y-6">
                    <!-- Document Upload (for CMAS and Entity users) -->
                    @if ($isCmasContext || ($isEntityContext && auth()->check() && auth()->user()->entities()->exists()))
        ...
        <!-- Invoices Tab (if in CMAS or Federation context and has payment documents) -->
        @if (($isCmasContext || $isFederationContext) && $payment_documents && $payment_documents->isNotEmpty())
        ...
        <!-- Diving Tab (if in CMAS context and has diving data) -->
        @if ($isCmasContext && ($individual->divingProfessionalCertifications->count() > 0 || $individual->divingTechnicalDirectorAssignments()->where('status_class', 'Domain\Diving\States\AssignedDivingTechnicalDirectorState')->count() > 0))
            <x-slot name="tab_diving">
                <div class="space-y-6">
                    <!-- Non-CMAS Diving Certifications -->

                        ...
                        <div class="mb-4">
                            ...
                            <select wire:model="selectedFederationId" class="form-select mt-1 block w-full">
                                ...
                                @foreach($customerFederations as $federation)
                                    <option value="{{ $federation->id }}">{{ $federation->code_cmas }}</option>
                                @endforeach
                            </select>
                        </div>
                    @endif
                    ...
                    @if($customerType === 'individual')
                                        ...
                                        @foreach($individualSearchResults as $individual)
                                            <li wire:click="selectIndividual('{{ $individual['id'] }}', '{{ addslashes($individual['name']) }}')"
                                                ...
                                                <div class="font-semibold">
                                                    {{ $individual['code_cmas'] }} – {{ $individual['name'] }}
                                                </div>
                        ...
                        @if($selectedIndividualData)
                            <div class="mb-4 bg-white border rounded-lg shadow-sm overflow-hidden">
                                <!-- Header with CMAS ID and Status -->
                                <div class="bg-gray-50 px-4 py-2 border-b flex justify-between items-center">
                                    <div class="flex items-center space-x-2">
                                        <span class="text-xs font-medium text-gray-500">CMAS ID:</span>
                                        <span
                                            class="text-sm font-mono font-bold text-gray-900">{{ $selectedIndividualData['code_cmas'] }}</span>
                                    </div>
                ...
                <div style="background-color: rgba(0, 0, 0, 0.8)"
                    ...
                    <div class="p-4 max-w-xl mx-auto relative mt-24">
                        ...
                        <div class="shadow w-full rounded-lg bg-white overflow-hidden block p-8">
                            ...
                            <div class="mb-4">
                                <label
                                    class="block text-gray-800 font-bold text-sm uppercase tracking-wide">{{ __('documents.products') }}</label>
                                <select class="form-select w-full" wire:model="selectedProductId">
                                    <option value="">{{ __('documents.select_product') }}</option>
                                    @foreach($products as $product)
                                        <option value="{{ $product->id }}">{{ $product->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="text-gray-800 block font-bold text-sm uppercase tracking-wide">{{ __('documents.or') }}</div>

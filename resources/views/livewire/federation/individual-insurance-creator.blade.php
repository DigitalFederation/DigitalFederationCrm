<div>
    ...
    <div class="mb-6">
        <div class="flex justify-between items-center">
            <h1 class="page-first-title">{{ __('federation.create_individual_insurance') }}</h1>
            <a href="{{ route('federation.individual-insurances.index') }}" class="btn btn-secondary">
        ...
        <div class="information-box mt-4">
            <div class="flex items-center">
                ...
                <span class="text-sm text-blue-800">
                    {{ __('federation.insurance_creation_info') }}
                </span>
            </div>
        </div>
    </div>
            ...
            @if($selectedEntity)
                <div class="mt-4 p-4 bg-blue-50 rounded-lg">
                    <div class="flex items-center">
                        @svg('heroicon-o-information-circle', 'w-5 h-5 text-blue-600 mr-2')
                        <span class="text-sm text-blue-800">
                            {{ __('federation.facilitating_insurance_for_entity', ['entity' => $selectedEntity->name]) }}
                        </span>
                    </div>
                </div>
            @endif
    ...
    @if($selectedEntity)
        ...
        {{-- Insurance Package Selection Section --}}
        <x-ui.card class="mb-6">
            <div class="p-6">
                <h2 class="text-lg font-medium mb-4 flex items-center text-gray-900">
                    @svg('heroicon-o-shield-check', 'w-5 h-5 mr-2')
                    {{ __('federation.select_insurance_package') }}
                </h2>
                ...
                @if($packages->isEmpty())
                    <div class="text-center py-8 text-gray-500">
                        ...
                        <h3 class="mt-2 text-sm font-medium text-gray-900">{{ __('federation.no_insurance_packages_available') }}</h3>
                        <p class="mt-1 text-sm text-gray-500">{{ __('federation.no_insurance_packages_for_entity') }}</p>
                    </div>
                @else
                    <x-ui.card-grid columns="3" gap="lg">
                        @foreach($packages as $package)
                            <div class="bg-white rounded-lg shadow-lg border border-slate-200 overflow-hidden transition-all duration-300 h-full flex flex-col {{ $hasValidationPlanPrivileges ? 'hover:shadow-xl cursor-pointer' : 'opacity-60 cursor-not-allowed' }} {{ $selectedPackageId === $package->id ? 'ring-2 ring-green-500 bg-green-50' : '' }}"
                                 @if($hasValidationPlanPrivileges) wire:click="$set('selectedPackageId', {{ $package->id }})" @endif>
                                <!-- Green Header for Insurance -->
                                ...
                                <div class="flex-1 p-6 flex flex-col">
                                    ...
                                    <div class="flex-1">
                                        <h4 class="text-sm font-semibold text-slate-900 mb-3 flex items-center">
                                            ...
                                            {{ __('federation.insurance_coverage') }}
                                        </h4>
        ...
        @if($selectedPackageId)
            {{-- Members Selection Section (similar to membership creator but for insurance) --}}
            <x-ui.card>
                <div class="p-6">
                    <h2 class="text-lg font-medium mb-4 flex items-center text-gray-900">
                        @svg('heroicon-o-users', 'w-5 h-5 mr-2')
                        {{ __('federation.select_members_for_insurance') }}
                    </h2>
                    ...
                    <div class="mb-4 sm:flex sm:justify-between sm:items-center space-y-4 sm:space-y-0">
                        ...
                        <div class="flex items-center space-x-4">
                            <select wire:model.live="statusFilter" class="form-select text-sm">
                                ...
                                <option value="active">{{ __('federation.filter.active_insurance') }}</option>
                                <option value="inactive">{{ __('federation.filter.no_insurance') }}</option>
                            </select>
                        </div>
                    </div>

                    {{-- Members Table (similar structure to membership creator) --}}
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-gray-50">
                                <tr>
                                    ...
                                    <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer" wire:click="sort('code_cmas')">
                                        <div class="flex items-center space-x-1">
                                            <span>{{ __('federation.member_number') }}</span>
                                            <x-tables.sort-indicator :field="$sortField" :direction="$sortDirection" :name="'code_cmas'" />
                                        </div>
                                    </th>
                                </tr>
                            </thead>
                    ...
                    <div class="mt-6 flex justify-end space-x-3">
                        ...
                        <button
                            ...
                            @class([
                                ...
                                'bg-green-600 hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500' => !empty($selectedIndividuals) && $hasValidationPlanPrivileges,
                        ...
                        >
                            {{ __('federation.actions_buttons.assign_insurance_selected', ['count' => count($selectedIndividuals)]) }}
                        </button>
                    </div>
                </div>
            </x-ui.card>
    ...
    {{-- Confirmation Modal --}}
    <x-dialog-modal wire:model.live="confirmingAssignment">
        <x-slot name="title">
            {{ __('federation.modal.confirm_insurance_title') }}
        </x-slot>

        <x-slot name="content">
            <div class="space-y-4">
                <p>{{ __('federation.modal.confirm_insurance_message_for_entity', ['entity' => $selectedEntity?->name]) }}</p>
                ...
                <p class="text-sm text-gray-500">
                    {{ __('federation.modal.insurance_count_for_entity', [
                ...
                @if($errors->has('assignment'))
                    <div class="text-sm text-red-600">{{ $errors->first('assignment') }}</div>
                @endif
            </div>
        </x-slot>

        <x-slot name="footer">
            <x-secondary-button wire:click="$set('confirmingAssignment', false)" wire:loading.attr="disabled">
                {{ __('federation.actions_buttons.cancel') }}
            </x-secondary-button>

            <x-button class="ml-3 bg-green-600 hover:bg-green-700 focus:ring-green-500" wire:click="processAssignment" wire:loading.attr="disabled">

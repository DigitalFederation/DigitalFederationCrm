<div class="previous-layout-classes">
    ...
    <div class="mb-8 flex justify-between">
        <h1 class="page-first-title">
            @if(!empty($committee))
                {{ __('attachments.committee_files_title', ['committee' => __('attachments.committees.' . $committee->code)]) }}
            @else
        ...
        <form wire:submit="saveAttachment">
            ...
            <div class="flex flex-col md:flex-row justify-start items-start gap-x-4 mb-4">
                ...
                <div class="w-full md:w-1/3 flex flex-col">
                    ...
                    <select class="pr-6 rounded border border-solid border-neutral-300 form-select"
                            wire:model.live="selected_language">
            ...
            @if(!empty($recipient) && $recipient == 'federation')
                <div class="border-t border-slate-400 pt-4">
                    <x-information-box
                        title="Attention"
                        :body="__('Use the available options to filter Federations to whom you want to allow access to the file. Filter by Country or choose a specific record from the list.')"></x-information-box>
                    <section
                        class="flex flex-col md:flex-row justify-between items-center gap-x-4 my-4">
                        ...
                        <div class="w-full md:w-2/5 flex flex-col">
                            ...
                            @else
                                <p class="text-slate-600 ml-2 text-xs">{{ __('Selection disabled') }}</p>
                            @endif
                        </div>
                        <div class="w-full md:w-1/5 flex flex-col text-center">
                            <p> {{ __('-- OR --') }} </p>
                        </div>
                        <div class="w-full md:w-2/5 flex flex-col">
            ...
            @if(!empty($recipient) && $recipient == 'entity')
                <div class="border-t border-slate-400 pt-4">
                    <div class="flex information-box items-center gap-x-2 ">
                    ...
                    <section
                        class="flex flex-col md:flex-row justify-between items-center gap-x-4 my-4  pt-4">
                        ...
                        <div class="w-full md:w-2/5 flex flex-col">
                            ...
                            @if(!$this->disableEntitiesFederations)
                                <label class="block text-sm font-medium mb-1"
                                       for="entities_federations">{{ __('Federations') }} </label>
                                <livewire:input.select-multiple
                                    wire.model.live="entities_federations"
                                    :options="['maxItemCount' => 50]"
                                    :items="$federations"
                                    inputId="entities_federations"
                                    inputName="entities_federations[]"
                                    identifier="entities_federations" />
                            @else
                                <p class="text-slate-600 ml-2 text-xs">{{ __('Selection disabled') }}</p>
                            @endif
                        </div>
            ...
            @if($this->recipient == 'individual')
                <div class="border-t border-slate-400 pt-4">
                    <x-information-box
                        title="Attention"
                        :body="__('Use the available option to filter Individuals to whom you need to allow access to the file.')"></x-information-box>
                    <section
                        class="flex flex-col md:flex-row justify-between items-center gap-x-4 my-4">
                        ...
                        <table class="w-full table-auto border border-slate-400">
                            ...
                            @foreach($this->federations_preview as $federation)
                                <tr>
                                    ...
                                    <td class="border border-slate-700 p-2 text-sm">{{ $federation->code_cmas }}</td>
                                </tr>
                            @endforeach
                            </tbody>
                        </table>
            ...
            <section class="border-t border-slate-300 pt-6 mt-6">
                ...
                <div class="flex self-end gap-x-4">
                    ...
                    <a class="btn self-center bg-slate-500 text-white" href="{{ route('admin.dashboard') }}">
                        {{ __('Back') }}
                    </a>

                    <button
                        type="submit"
                        class="btn btn-action px-3"

            ...
            @if ($hasDivingContent)
                <section id="mergulho" class="mb-12 scroll-mt-20">
                    ...
                    @if ($featuredSpots->isNotEmpty())
                        <div class="mb-8">
                            <h3 class="text-lg font-semibold text-gray-800 dark:text-gray-200 mb-4">
                                {{ __('public.entity.nearby_diving_spots') }}
                            </h3>
                            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                                @foreach ($featuredSpots as $spot)
                                    <div
                                        ...
                                        <div class="px-6 py-5">
                                            <div class="flex items-center justify-between mb-3">
                                                <div class="flex items-center">
                                                    @if ($spot['country_code'])
                                                        <span
                                                            class="fi fi-{{ strtolower($spot['country_code']) }} rounded-sm shadow-sm mr-2"></span>
                                                    @endif
                                            ...
                                            <div class="mt-4">
                                                <a href="{{ $spot['url'] }}"
                                                    class="inline-flex items-center justify-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-colors w-full">
                                                    {{ __('View on Map') }}
                                                </a>
                                            </div>
                                        </div>
                    ...
                    @if ($availableCourses->isNotEmpty())
                        <div>
                            <h3 class="text-lg font-semibold text-gray-800 dark:text-gray-200 mb-4">
                                {{ __('public.entity.available_courses') }}
                            </h3>
                            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                @foreach ($availableCourses as $course)
                                    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-md overflow-hidden entity-card border border-gray-100 dark:border-gray-700 transition-all hover:shadow-lg flex flex-col">
                                        {{-- Course Image --}}
                                        <div class="aspect-video bg-gradient-to-br from-blue-50 to-blue-100 dark:from-blue-900 dark:to-blue-800 relative">
                                            ...
                                            @elseif ($course->certification && $course->certification->certification_view)
                                                <img src="{{ asset('storage/img/cards/' . $course->certification->certification_view) }}"
                                                    alt="{{ $course->display_name }}"
                                                    class="w-full h-full object-contain p-4">
                                            @else
                                                <div class="w-full h-full flex items-center justify-center">
                                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-16 w-16 text-blue-300 dark:text-blue-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            ...
                                            @endif
                                            {{-- Certification System Badge --}}
                                            @if ($course->certification_system)
                                                <div class="absolute top-3 right-3">
                                                    <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold bg-white/90 dark:bg-gray-900/90 text-blue-700 dark:text-blue-300 shadow-sm backdrop-blur-sm">
                                        ...
                                        <div class="p-5 flex-1 flex flex-col">
                                            ...
                                            <div class="mb-3 flex flex-wrap gap-2">
                                                @if ($course->start_date)
                                                    <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium bg-blue-100 text-blue-800 dark:bg-blue-900/50 dark:text-blue-200">
                                                        ...
                                                        {{ __('Starts:') }} {{ $course->start_date->format('d M Y') }}
                                                    </span>
                                                @else
                                            ...
                                            @if ($course->district || $course->location)
                                                <div class="mb-3">
                                                    <p class="text-sm text-gray-600 dark:text-gray-400 flex items-center">
                                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-1.5 text-gray-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                                                            <path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"></path>
                                                            <circle cx="12" cy="10" r="3"></circle>
                                                        </svg>
                                                        @if ($course->location && $course->district)
                                            ...
                                            {{-- Course Details (only if certification exists) --}}
                                            @if ($course->certification)
                                                <div class="grid grid-cols-2 gap-2 mb-3 text-center">
                                                    @if ($course->certification->minimum_age)
                                                        <div class="bg-gray-50 dark:bg-gray-700/50 rounded-lg p-2">
                                                            <span class="block text-xs text-gray-500 dark:text-gray-400">{{ __('Min. Age') }}</span>
                                                            <span class="font-semibold text-sm text-gray-900 dark:text-white">{{ $course->certification->minimum_age }}</span>
                                                        </div>
                                                    @endif
                                                    @if ($course->certification->open_water_sessions)
                                                        <div class="bg-gray-50 dark:bg-gray-700/50 rounded-lg p-2">
                                                            <span class="block text-xs text-gray-500 dark:text-gray-400">{{ __('Open Water') }}</span>
                                                            <span class="font-semibold text-sm text-gray-900 dark:text-white">{{ $course->certification->open_water_sessions }}</span>
                                                        </div>
                                                    @endif
                                                </div>
                                            @endif
                                            ...
                                            @if ($course->about)
                                                <div class="flex-1">
                                                    <p class="text-sm text-gray-600 dark:text-gray-400 line-clamp-3">
                                                        {{ Str::limit(strip_tags($course->about), 150) }}
                                                    </p>
                                                </div>
                                            @endif
                                            ...
                                            @if ($entity->email)
                                                <div class="mt-4 pt-3 border-t border-gray-100 dark:border-gray-700">
                                                    <a href="mailto:{{ $entity->email }}?subject={{ urlencode(__('public.entity.course_inquiry_subject', ['course' => $course->display_name])) }}"
                                                       class="w-full inline-flex items-center justify-center px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium rounded-lg transition-colors">
                                                        ...
                                                        {{ __('public.entity.contact_school') }}
                                                    </a>
                                                </div>
                                            @endif
                                        </div>
    ...
    @push('footer-scripts')
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                ...
                // const entityName = "{{ addslashes($entity->name) }}"; // Removed duplicate
                const shareText = `{{ __('Check out this entity on :portal:', ['portal' => config('branding.primary.portal_name', 'Digi Federation')]) }} ${entityName}`;

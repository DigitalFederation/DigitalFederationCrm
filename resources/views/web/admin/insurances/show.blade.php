@section('title', __('main.insurance_details'))

<x-layout>
    <div class="min-h-screen">
        @php
            $insurancePlan = $insurance->insurancePlan;
            $policyNumber = $insurance->policy_number ?? $insurancePlan->policy_number ?? null;
            ...
            $territorialScope = $insurancePlan->territorial_scope ?? null;
            $insuredActivity = $insurancePlan->insured_activity ?? null;
        @endphp

        <!-- Back Button -->
        <div class="mb-4">
            <a href="{{ route('admin.insurances.index') }}" class="inline-flex items-center text-sm text-slate-600 hover:text-slate-900 transition-colors">
                ...
                <div class="px-6 py-5 {{ $isActive && !$isExpired ? 'bg-gradient-to-r from-slate-700 to-slate-600' : ($isExpired ? 'bg-gradient-to-r from-slate-600 to-slate-500' : 'bg-gradient-to-r from-amber-500 to-amber-400') }}">
                    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                        ...
                        <div class="flex flex-wrap gap-2">
                            <a href="{{ route('admin.insurances.edit', $insurance) }}" class="inline-flex items-center px-4 py-2 bg-white/20 text-white rounded-lg font-medium text-sm hover:bg-white/30 transition-colors backdrop-blur-sm">
                                <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                                </svg>
                                {{ __('main.edit') }}
                            </a>
                            <a href="{{ route('admin.insurances.document.download', $insurance) }}" class="inline-flex items-center px-4 py-2 bg-white text-slate-800 rounded-lg font-medium text-sm hover:bg-slate-50 transition-colors shadow-sm">
                                <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                                </svg>
                                {{ __('insurance.download_insurance') }}
                            </a>
                            @if($hasConditionsDocument)
                                <a href="{{ route('admin.insurances.document.conditions', $insurance) }}" class="inline-flex items-center px-4 py-2 bg-white/20 text-white rounded-lg font-medium text-sm hover:bg-white/30 transition-colors backdrop-blur-sm">
                                    <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div class="px-6 py-4 border-b border-slate-100 bg-slate-50">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p class="text-xs text-slate-500">{{ __('insurance.insured_personal_information') }}</p>
                            </div>
                        </div>
                    </div>
                    <div class="p-6">
                        ...
                        @if($entity)
                            <div class="mt-6 pt-6 border-t border-slate-100">
                                <h4 class="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-4">{{ __('federation.individual_insurances.associated_entity') }}</h4>
                                <div class="flex items-center gap-3 p-3 bg-slate-50 rounded-lg">
                                    ...
                                    <div>
                                        ...
                                        @if($entity->code_internal)
                                            <dd class="text-xs text-slate-500">{{ $entity->code_internal }}</dd>
                                        @endif
                                    </div>
                                </div>
                            </div>
                    ...
                    <div class="px-6 py-4 border-b border-slate-100 bg-slate-50">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p class="text-xs text-slate-500">{{ __('insurance.plan_details') }}</p>
                            </div>
                        </div>
                    </div>
                    <div class="p-6">
                        ...
                        <div class="mt-6 pt-6 border-t border-slate-100">
                            ...
                            @php
                                $daysRemaining = (int) $insurance->end_date->diffInDays(now(), false);
                            @endphp
                        ...
                        <!-- Coverage Details -->
                        @if($territorialScope || $insuredActivity)
                            <div class="mt-6 pt-6 border-t border-slate-100">
                                <h4 class="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-4">{{ __('insurance.coverage_details') }}</h4>
                                <div class="space-y-3">
                                    @if($territorialScope)
                                        <div class="flex items-start gap-3 p-3 bg-slate-50 rounded-lg">
                                            <svg class="w-5 h-5 text-slate-500 mt-0.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <div class="px-6 py-4 border-b border-slate-100 bg-slate-50">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p class="text-xs text-slate-500">{{ __('insurance.insurer_information') }}</p>
                                </div>
                            </div>
                        </div>
                        <div class="p-6">

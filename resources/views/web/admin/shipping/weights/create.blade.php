<x-layout>
    <div class="previous-layout-classes">
        ...
        <div class="mb-8 flex justify-between">
            <h1 class="page-first-title">{{ __('Create Shipping Weight') }}</h1>
        </div>

        <form action="{{ route('admin.shipping.weights.store') }}" method="POST">
            @csrf
                            ...
                            <div class="flex flex-wrap -mx-4 space-y-4 md:space-y-0">
                                ...
                                <div class="w-full px-4 md:w-1/4">
                                    ...
                                    <select id="method_id" name="method_id" class="form-select w-full" required>
                                        ...
                                        @foreach ($shippingMethods as $method)
                                            <option
                                                value="{{ $method->id }}" {{ old('method_id') == $method->id ? 'selected' : '' }}>{{ $method->name }}</option>
                                        @endforeach
                                    </select>
                                    @error('method_id')
                                ...
                                <div class="w-full px-4 md:w-1/4">
                                    ...
                                    <input id="minimum_weight" class="form-input w-full" type="text"
                                           name="minimum_weight" value="{{ old('minimum_weight') }}" required />
                                ...
                                <div class="w-full px-4 md:w-1/4">
                                    ...
                                    <input id="maximum_weight" class="form-input w-full" type="text"
                                           name="maximum_weight" value="{{ old('maximum_weight') }}" required />
                        ...
                        <section>
                            <div class="flex flex-col px-6 py-5 border-t border-slate-200">
                                <div class="flex self-end">
                                    ...
                                    <button type="submit"
                                            class="btn bg-blue-500 hover:bg-blue-600 text-white ml-3">{{ __('Create Shipping Weight') }}</button>
                                </div>
                            </div>
                        </section>
                    </div>

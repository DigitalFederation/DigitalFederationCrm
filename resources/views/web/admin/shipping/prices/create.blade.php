<x-layout>
    <div class="previous-layout-classes">
        ...
        <div class="mb-8 flex justify-between">
            <h1 class="page-first-title">{{ __('Create Shipping Price') }}</h1>
        </div>
                        ...
                        <section class="pb-4 space-y-6">
                            ...
                            <div class="flex flex-wrap -mx-4 space-y-4 md:space-y-0 gap-y-4">
                                ...
                                <div class="w-full px-4 md:w-1/2">
                                    ...
                                    <select name="weight_id" id="weight_id" class="form-select w-full" required>
                                        @foreach($weights as $weight)
                                            <option value="{{ $weight->id }}">{{ $weight->range }}</option>
                                        @endforeach
                                    </select>
                                    @error('weight_id')
                                    <div class="text-xs mt-1 text-rose-500">{{ $message }}</div>
                                ...
                                <div class="w-full px-4 md:w-1/2">
                                    ...
                                    <input id="price" class="form-input w-full" type="text" name="price" value="{{ old('price') }}" required />
                                    @error('price')
                                    <div class="text-xs mt-1 text-rose-500">{{ $message }}</div>
                                    @enderror
                                </div>

<x-layout>
    <div class="previous-layout-classes">
        ...
        <div class="mb-8 flex justify-between">
            <h1 class="page-first-title">{{ __('Add Diving Course') }}</h1>
        </div>

        <form action="{{ route('admin.diving-course.store') }}" method="POST" class="card p-6 max-w-2xl mx-auto">
            @csrf
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="certification_id" id="certification_id" class="form-input w-full">
                        ...
                        @foreach($certifications as $certification)
                            <option value="{{ $certification->id }}" @if(old('certification_id') == $certification->id) selected @endif>{{ $certification->name }}</option>
                        @endforeach
                    </select>
                ...
                <div class="sm:col-span-2">
                    ...
                    <textarea name="about" id="about" rows="5" class="form-input w-full">{{ old('about') }}</textarea>
            ...
            <div class="flex gap-4 justify-end mt-8">
                ...
                <button type="submit" class="btn btn-action">{{ __('Save') }}</button>
            </div>
        </form>
    </div>
</x-layout>

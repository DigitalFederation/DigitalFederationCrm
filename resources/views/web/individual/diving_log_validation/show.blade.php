        ...
        <div class="sm:flex sm:justify-between sm:items-center mb-4">
            ...
            <div class="mb-4 sm:mb-0">
                <h1 class="page-first-title">
                    {{ __('diving_log.diving_log_review') }}
                </h1>
            </div>

            <!-- Right: Actions -->
            <div class="flex items-center">
                <a href="{{ route('entity.diving-log-validation.index') }}" class="btn btn-info">
                    {{ __('common.back') }}
                </a>
            </div>
        </div>
                    ...
                    <div class="w-full md:w-1/2">
                        @switch($divingLog->dive_details['type'])
                            ...
                            @case(\App\Enums\DivingLogDiveTypeEnum::ExtendedRange)
                                <x-diving_log.info-block-extended-range
                                    :divingLog="$divingLog->dive_details['details']" />
                                @break
                            ...
                            @case(\App\Enums\DivingLogDiveTypeEnum::RebreatherScr)
                                <x-diving_log.info-block-rebreather-scr
                                    :divingLog="$divingLog->dive_details['details']" />
                                @break
                            @case(\App\Enums\DivingLogDiveTypeEnum::RebreatherCcr)
                                <x-diving_log.info-block-rebreather-ccr
                                    :divingLog="$divingLog->dive_details['details']" />
                                @break
                        @endswitch
                    </div>
        ...
        @if($divingLog->status_class == \Domain\DivingLogs\States\PendingDivingLogState::class)
            <section>
                <form action="{{ route('individual.diving-log-validation.update', $divingLog->id) }}" method="POST"
                    ...
                    @csrf
                    @method('PUT')
                    <button type="submit" class="btn btn-primary text-xl w-full p-4">{{ __('diving_log.approve_dive') }}</button>
                </form>
            </section>
        @endif

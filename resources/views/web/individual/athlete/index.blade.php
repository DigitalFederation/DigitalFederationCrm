        ...
        <div class="sm:flex sm:justify-between sm:items-center mb-6">
            <div class="mb-4 sm:mb-0">
                ...
                <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">
                    {{ __('athletes.manage_club_associations') }}
                </p>
            </div>
            ...
            <div class="border-b border-slate-200 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800/50">
                <nav class="flex gap-x-1 px-4" aria-label="Tabs">
                    ...
                    >
                        ...
                        <span>{{ __('athletes.associated_entities') }}</span>
                        @if($activeCount > 0)
                            <span
                    ...
                    >
                        ...
                        <span>{{ __('athletes.entity_invitations') }}</span>
                        @if($pendingCount > 0)
                            <span
                    ...
                    >
                        ...
                        <span>{{ __('athletes.rejected') }}</span>
                        @if($historyCount > 0)
                            <span
                        ...
                        <div class="overflow-x-auto rounded-lg border border-slate-200 dark:border-slate-600">
                            <table class="min-w-full divide-y divide-slate-200 dark:divide-slate-600">
                                ...
                                <tbody class="divide-y divide-slate-200 dark:divide-slate-600 bg-white dark:bg-slate-800">
                                    @foreach($activeInvites as $athlete)
                                        @php
                                        ...
                                        <tr class="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                                                ...
                                                <div class="flex items-center gap-3">
                                                    <div class="flex-shrink-0 w-9 h-9 rounded-lg bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center shadow-sm">
                                                        <span class="text-white font-bold text-xs">
                                                            {{ strtoupper(substr($athlete->entity->name, 0, 2)) }}
                                                        </span>
                                                    </div>
                                                    <div>
                                            ...
                                            <td class="px-5 py-4 whitespace-nowrap">
                                                <x-tables.badge :status="$athlete->getLicenseStatusName()" :color="$athlete->getLicenseStatusColor()" />
                                            </td>
                                            <td class="px-5 py-4 whitespace-nowrap text-sm text-slate-500 dark:text-slate-400">
                                                @if($sportLicense && $sportLicense->current_term_ends_at)
                                                    {{ \Carbon\Carbon::parse($sportLicense->current_term_ends_at)->format('d M Y') }}
                                                @else
                                                    -
                                                @endif
                                            </td>
                                            <td class="px-5 py-4 whitespace-nowrap text-right">
                                                <form
                                                    action="{{ route('individual.athlete.delete', $athlete->id) }}"
                                                    method="POST"
                                                    class="inline"
                                                    onsubmit="return confirm('{{ __('athletes.confirm_leave_entity') }}')"
                                                >
                                                    @csrf
                                                    @method('DELETE')
                                                    <button
                    ...
                    @else
                        ...
                        <div class="text-center py-12">
                            ...
                            <h3 class="text-base font-semibold text-slate-900 dark:text-white mb-1">
                                {{ __('athletes.no_active_associations') }}
                            </h3>
                            <p class="text-sm text-slate-500 dark:text-slate-400 max-w-sm mx-auto">
                                {{ __('athletes.no_active_associations_desc') }}
                            </p>
                            @if($pendingCount > 0)
                                <button
                                    @click="activeTab = 'pending'"
                                ...
                                >
                                    ...
                                    {{ __('athletes.view_pending_invitations') }}
                                </button>
                            @endif
                        </div>
                    @endif
                ...
                >
                    ...
                    @if($pendingInvites->count() > 0)
                        <div class="overflow-x-auto rounded-lg border border-slate-200 dark:border-slate-600">
                            <table class="min-w-full divide-y divide-slate-200 dark:divide-slate-600">
                                ...
                                <tbody class="divide-y divide-slate-200 dark:divide-slate-600 bg-white dark:bg-slate-800">
                                    @foreach($pendingInvites as $athlete)
                                        <tr class="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                                            <td class="px-5 py-4 whitespace-nowrap">
                                                <div class="flex items-center gap-3">
                                            ...
                                            <td class="px-5 py-4 whitespace-nowrap">
                                                <x-tables.badge :status="ucwords($athlete->stateName())" :color="$athlete->stateColor()" />
                                            </td>
                                            <td class="px-5 py-4 whitespace-nowrap text-right">
                                                @if($athlete->status_class === \Domain\Entities\States\PendingEntityProfessionalRoleState::class)
                                                    <div class="flex items-center justify-end gap-2">
                                                        <form action="{{ route('individual.athlete.response', $athlete->id) }}" method="POST">
                                                            @csrf
                                                            @method('PUT')
                                                ...
                                                @else
                                                    <form
                                                        ...
                                                        method="POST"
                                                        class="inline"
                                                        onsubmit="return confirm('{{ __('athletes.confirm_delete_invitation') }}')"
                                                    >
                                                        @csrf
                                                        @method('DELETE')
                                                        <button
                    ...
                    @else
                        ...
                        <div class="text-center py-12">
                            ...
                            <h3 class="text-base font-semibold text-slate-900 dark:text-white mb-1">
                                {{ __('athletes.no_pending_invitations') }}
                            </h3>
                            <p class="text-sm text-slate-500 dark:text-slate-400 max-w-sm mx-auto">
                                {{ __('athletes.all_caught_up') }}
                            </p>
                            @if($activeCount > 0)
                                <button
                                    @click="activeTab = 'active'"
                                    class="mt-4 inline-flex items-center gap-2 px-4 py-2 bg-indigo-500 hover:bg-indigo-600 text-white text-sm font-medium rounded-lg transition-colors"
                                >
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" />
                                    </svg>
                                    {{ __('athletes.view_active_associations') }}
                                </button>
                            @endif
                        </div>
                    @endif
                ...
                >
                    ...
                    @if($historyInvites->count() > 0)
                        <div class="overflow-x-auto rounded-lg border border-slate-200 dark:border-slate-600">
                            <table class="min-w-full divide-y divide-slate-200 dark:divide-slate-600">
                                ...
                                <tbody class="divide-y divide-slate-200 dark:divide-slate-600 bg-white dark:bg-slate-800">
                                    @foreach($historyInvites as $athlete)
                                        @php
                                            $isRejected = $athlete->status_class === \Domain\Entities\States\RejectedEntityProfessionalRoleState::class;
                                        ...
                                        <tr class="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                                            ...
                                            <td class="px-5 py-4 whitespace-nowrap">
                                                ...
                                                <div class="text-xs text-slate-500 dark:text-slate-400 mt-1">
                                                    {{ $athlete->updated_at->format('d M Y, H:i') }}
                                                </div>
                                            </td>
                                        </tr>
                                    @endforeach
                    ...
                    @else
                        ...
                        <div class="text-center py-12">
                            ...
                            <h3 class="text-base font-semibold text-slate-900 dark:text-white mb-1">
                                {{ __('athletes.no_rejected') }}
                            </h3>
                            <p class="text-sm text-slate-500 dark:text-slate-400 max-w-sm mx-auto">
                                {{ __('athletes.no_rejected_desc') }}
                            </p>
                        </div>
                    @endif
                </div>

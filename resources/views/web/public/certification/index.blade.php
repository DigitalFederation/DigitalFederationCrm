...
<x-public-layout>
                    ...
                    <div class="flex flex-col gap-6 p-6">
                        <div class="flex flex-col sm:flex-row items-start sm:items-end gap-4">
                            ...
                            <div class="flex-grow w-full">
                                <label for="code_cmas"
                                    class="block text-sm font-medium text-gray-700 mb-1">{{ __('main.CMAS Code') }}</label>
                                <input type="text" id="code_cmas" name="code_cmas"
                                    placeholder="{{ __('Enter Nº Filiado') }}"
                                    class="block w-full shadow-sm sm:text-sm border-gray-300 rounded-md focus:ring-sky-500 focus:border-sky-500 {{ $errors->has('code_cmas') ? 'border-rose-500' : '' }}"
                                    value="{{ old('code_cmas') }}">
                                @if ($errors->has('code_cmas'))
                                    <p class="mt-1 text-xs text-rose-500">{{ $errors->first('code_cmas') }}</p>
                                @endif
                            </div>
                        </div>
                        ...
                        <button type="submit"
                            ...
                            <span>{{ __('Search by CMAS Code') }}</span>
                        </button>
                    </div>
                </form>
    ...
    @push('footer-scripts')
        ...
        <script>
            ...
            const cmasCodeInput = document.getElementById('code_cmas');
            const form1 = cmasCodeInput.closest('form');
                ...
                const startScannerWithOptions = (options) => {
                    return qrCodeScanner.start(
                        ...
                        (decodedText, decodedResult) => { // success callback
                            cmasCodeInput.value = decodedText;

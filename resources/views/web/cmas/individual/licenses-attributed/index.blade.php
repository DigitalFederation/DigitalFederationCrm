@section('title', __('cmas.my_international_licenses'))
<x-layout>
    <div class="previous-layout-classes">
        ...
        <div class="sm:flex sm:justify-between sm:items-center mb-5 mt-5">
            ...
            <div class="mb-4 sm:mb-0">
                <h1 class="page-first-title">{{ __('cmas.my_international_licenses') }}</h1>
            </div>

            <!-- Right: Actions -->
            <div class="grid grid-flow-col sm:auto-cols-max justify-start sm:justify-end gap-2">
                <a href="{{ route('cmas.individual.license-purchase.index', ['committee' => $committee ?? 'diving']) }}"
                   class="btn btn-primary">
                    <svg class="w-4 h-4 fill-current opacity-50 shrink-0" viewBox="0 0 16 16">
                        <path d="M15 7H9V1c0-.6-.4-1-1-1S7 .4 7 1v6H1c-.6 0-1 .4-1 1s.4 1 1 1h6v6c0 .6.4 1 1 1s1-.4 1-1V9h6c.6 0 1-.4 1-1s-.4-1-1-1z" />
                    </svg>
                    <span class="ml-2">{{ __('cmas.purchase_license') }}</span>
                </a>
            </div>
                            ...
                            <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap w-px">
                                <div class="flex justify-end">
                                    <x-dynamic-table-buttons type="show"
                                                             :route="route('cmas.individual.licenses-attributed.show', $license->id)" />
                                </div>
                            </td>
                        </tr>
                    @endforeach
        ...
        @else
            <x-utility.no-data :title="__('cmas.no_licenses')" :inCard="true" />
        @endif

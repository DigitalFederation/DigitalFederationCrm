@section('title', __('cmas.my_certification_cards'))
<x-layout>
    <div class="previous-layout-classes">
        ...
        <div class="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4 mb-6 flex items-center">
            ...
            <div>
                <h2 class="text-lg font-semibold text-blue-900 dark:text-blue-100">{{ __('cmas.international_certifications') }}</h2>
                <p class="text-sm text-blue-700 dark:text-blue-300">{{ __('cmas.diving_scientific_certifications') }}</p>
            </div>
        </div>
        ...
        <div class="sm:flex sm:justify-between sm:items-center mb-8">
            <div class="mb-4 sm:mb-0">
                <h1 class="page-first-title">{{ __('cmas.my_certification_cards') }}</h1>
                <p class="text-slate-600 dark:text-slate-400">{{ __('cmas.download_certification_cards') }}</p>
            </div>
        </div>

        <x-information-box
            :title="__('cmas.information')"
            :body="__('cmas.certifications_information')" />
                            ...
                            <div class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg p-4 hover:shadow-lg transition-shadow">
                                ...
                                <div class="text-xs text-slate-600 dark:text-slate-400 space-y-1 mb-3">
                                    @if($certification->current_term_starts_at)
                                        <p>{{ __('cmas.issued') }}: {{ $certification->current_term_starts_at->format('d/m/Y') }}</p>
                                    @endif
                                    @if($certification->current_term_ends_at)
                                        <p>{{ __('cmas.expires') }}: {{ $certification->current_term_ends_at->format('d/m/Y') }}</p>
                                    @endif
                                    @if($certification->cmas_international_code)
                                        <p>{{ __('cmas.code') }}: {{ $certification->cmas_international_code }}</p>
                                    @endif
                                </div>

                                <div class="flex gap-2">
                                    <a href="{{ route('cmas.individual.certification-card.show', $certification->id) }}"
                                       class="flex-1 btn btn-secondary text-xs py-1">
                                        {{ __('cmas.view') }}
                                    </a>
                                    <a href="{{ route('cmas.individual.certification-card.download', $certification->id) }}"
        ...
        @else
            <div class="bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg p-8 text-center">
                ...
                <h3 class="mt-2 text-sm font-medium text-slate-900 dark:text-slate-100">{{ __('cmas.no_certifications') }}</h3>
                <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">{{ __('cmas.no_certifications_description') }}</p>
            </div>
        @endif

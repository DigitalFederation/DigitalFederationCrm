@section('title', __('cmas.manage_international_certifications'))
<x-layout>
    <div class="previous-layout-classes">
        ...
        <div class="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4 mb-6 flex items-center">
            ...
            <div>
                <h2 class="text-lg font-semibold text-blue-900 dark:text-blue-100">{{ __('cmas.international_certifications_management') }}</h2>
                <p class="text-sm text-blue-700 dark:text-blue-300">{{ __('cmas.manage_federation_international_certifications') }}</p>
            </div>
        </div>
        ...
        <div class="sm:flex sm:justify-between sm:items-center mb-8">
            <div class="mb-4 sm:mb-0">
                <h1 class="page-first-title">{{ __('cmas.international_certifications') }}</h1>
                <p class="text-slate-600 dark:text-slate-400">{{ __('cmas.federation_certifications_description') }}</p>
            </div>
        </div>
                    ...
                    @foreach($certifications_attributed as $certification)
                        <tr>
                            <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap w-px">
                                <a href="{{ route('cmas.federation.certifications-attributed.show', $certification->id) }}"
        ...
        @else
            <div class="bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg p-8 text-center">
                ...
                <h3 class="mt-2 text-sm font-medium text-slate-900 dark:text-slate-100">{{ __('cmas.no_certifications_found') }}</h3>
                <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">{{ __('cmas.no_federation_certifications_description') }}</p>
            </div>
        @endif

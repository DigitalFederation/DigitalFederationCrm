@section('title', __('federation.individual_insurances.insurance_details'))
...
<x-layout>
    <div class="min-h-screen">
        ...
        <!-- Back Button -->
        <div class="mb-4">
            <a href="{{ route('federation.individual-insurances.index') }}" class="inline-flex items-center text-sm text-slate-600 hover:text-slate-900 transition-colors">
                ...
                <div class="px-6 py-5 {{ $isActive && !$isExpired ? 'bg-gradient-to-r from-slate-700 to-slate-600' : ($isExpired ? 'bg-gradient-to-r from-slate-600 to-slate-500' : 'bg-gradient-to-r from-amber-500 to-amber-400') }}">
                    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                        ...
                        <div class="flex flex-wrap gap-2">
                            <a href="{{ route('federation.insurances.document.download', $insurance) }}" class="inline-flex items-center px-4 py-2 bg-white text-slate-800 rounded-lg font-medium text-sm hover:bg-slate-50 transition-colors shadow-sm">
                            ...
                            @if($hasConditionsDocument)
                                <a href="{{ route('federation.insurances.document.conditions', $insurance) }}" class="inline-flex items-center px-4 py-2 bg-white/20 text-white rounded-lg font-medium text-sm hover:bg-white/30 transition-colors backdrop-blur-sm">
                                    <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">

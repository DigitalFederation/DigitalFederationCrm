    ...
    <div class="previous-layout-classes">
        ...
        <div class="sm:flex sm:justify-between sm:items-center mb-4">
            ...
            <div class="grid grid-flow-col sm:auto-cols-max justify-start sm:justify-end gap-2">
                @php
                    // Determine the correct purchase route based on context
                    // Committee codes are uppercase: SPORT, DIVING, DIVINGSERVICES, SCIENTIFIC
                    $isMembers = !empty($filterHolderType) && $filterHolderType === 'individual';
                    $isSeparatedRoute = isset($isInternational);

                    if ($isSeparatedRoute && !empty($committee)) {
                        // Use separated purchase routes
                        if ($committee === 'SPORT' && !($isInternational ?? false)) {
                            ...
                            $purchaseRoute = $isMembers
                                ? route('entity.national-diving-member-license-purchase.index')
                                : route('entity.diving_licenses.request');
                        } elseif ($committee === 'SCIENTIFIC' && ($isInternational ?? false)) {
                            $purchaseRoute = $isMembers
                                ? route('entity.scientific-member-license-purchase.index')
                                : route('entity.scientific-license-purchase.index');
                        } else {
                            // Fallback to sport license purchase
                            $purchaseRoute = $isMembers
                                ? route('entity.sport-member-license-purchase.index')
                                : route('entity.sport-license-purchase.index');
                        }
        ...
        <div class="sm:flex flex-row gap-4">
            ...
            <x-filter-form :post="route('entity.license-attributed.index')">
                ...
                <x-forms.filter-input-text label="{{ __('main.CMAS Code') }}" name="filter_cmas_code" />
                        ...
                        @foreach($licenses as $license)
                            <tr>
                                ...
                                @if($filterHolderType === 'individual')
                                    ...
                                    <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap w-px">{{ $owner->code_cmas ?? '-' }}</td>
                                @endif
                                <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap w-px">

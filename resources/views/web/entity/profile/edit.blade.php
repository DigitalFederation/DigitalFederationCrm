                ...
                <div class="col-span-12 lg:col-span-4 space-y-6">
                    ...
                    <div class="bg-white shadow-sm rounded-lg">
                        <div class="p-6">
                            ...
                            <!-- Featured Diving Locations -->
                            @role('entity-diving-services')
                                <div>
                                    <label for="featured_diving_locations"
                                        class="block text-sm font-medium text-gray-700 mb-2">
                                        {{ __('Nearby Diving Locations') }}
                                    </label>
                                    <div class="mt-1">
                                        <livewire:input.select-multiple :items="$divingLocations" :input-selected="old('featured_diving_locations', $featuredLocationIds)"
                                            input-name="featured_diving_locations[]" input-id="featured_diving_locations"
                                            identifier="featured-locations" />
                                    </div>
                                    @if ($errors->has('featured_diving_locations') || $errors->has('featured_diving_locations.*'))
                                        <p class="mt-1 text-sm text-red-600">
                                            {{ $errors->first('featured_diving_locations') ?: $errors->first('featured_diving_locations.*') }}
                                        </p>
                                    @else
                                        <p class="mt-1 text-xs text-gray-500">
                                            {{ __('Select the diving spots you want to highlight on your profile.') }}
                                        </p>
                                    @endif
                                </div>
                            @endrole

@section('title', __('scientific.instructor_leader_associated_entities'))
<x-layout>
        ...
        <div class="sm:flex sm:justify-between sm:items-center mb-6">
            ...
            <div class="mb-4 sm:mb-0">
                <div class="flex items-center gap-3">
                    <h1 class="text-2xl font-bold text-slate-900 dark:text-white tracking-tight">{{ __('scientific.scientific_instructors_leaders') }}</h1>
                ...
                <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">
                    {{ __('scientific.associated_to') }} <span class="font-semibold text-slate-700 dark:text-slate-300">{{ $entity->name }}</span>
                </p>
            </div>
            ...
            <div class="flex items-center gap-3">
                ...
                >
                    ...
                    <span>{{ __('scientific.invite_instructor') }}</span>
                </button>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            ...
            <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 p-5">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        <p class="text-sm font-medium text-slate-500 dark:text-slate-400">{{ __('scientific.total_instructors') }}</p>
            ...
            <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 p-5">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        <p class="text-sm font-medium text-slate-500 dark:text-slate-400">{{ __('scientific.active_instructors') }}</p>
            ...
            <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 p-5">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        <p class="text-sm font-medium text-slate-500 dark:text-slate-400">{{ __('scientific.pending_invitations') }}</p>
            ...
            <div class="border-b border-slate-200 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800/50">
                <nav class="flex gap-x-1 px-4" aria-label="Tabs">
                    ...
                    >
                        ...
                        <span>{{ __('scientific.associated_instructors') }}</span>
                        @if($instructors->total() > 0)
                            <span
                    ...
                    >
                        ...
                        <span>{{ __('scientific.pending_invitations_tab') }}</span>
                        <span
                ...
                <div x-show="activeTab === 'active'" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" x-cloak>
                                ...
                                <thead>
                                    <tr class="bg-slate-50 dark:bg-slate-700/50">
                                        <th scope="col" class="px-5 py-3 text-left text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">
                                            {{ __('scientific.instructor') }}
                                        </th>
                                        <th scope="col" class="px-5 py-3 text-left text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">
                                            {{ __('scientific.role') }}
                                        </th>
                                        <th scope="col" class="px-5 py-3 text-left text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">
                                            {{ __('scientific.joined') }}
                                        </th>
                                        <th scope="col" class="px-5 py-3 text-left text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">
                                            {{ __('scientific.status') }}
                                        </th>
                                        ...
                                        <tr class="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                                            ...
                                            <td class="px-5 py-4 whitespace-nowrap text-right text-sm font-medium">
                                                <div class="flex items-center justify-end gap-2">
                                                    ...
                                                    <x-dynamic-table-buttons
                                                        type="disassociate"
                                                        :route="route('entity.scientific-instructor.remove', $instructor->id)"
                                                        method="DELETE"
                                                        :confirmText="__('scientific.confirm_disassociate')"
                                                    />
                                                </div>
                                            </td>
                                        </tr>
                    ...
                    @else
                        ...
                        <div class="text-center py-12">
                            ...
                            <h3 class="text-base font-semibold text-slate-900 dark:text-white mb-1">{{ __('scientific.no_instructors') }}</h3>
                            <p class="text-sm text-slate-500 dark:text-slate-400">{{ __('scientific.start_by_inviting') }}</p>
                            <div class="mt-4">
                                <button @click="showInviteModal = true" class="btn btn-primary">
                                    {{ __('scientific.invite_first_instructor') }}
                                </button>
                            </div>
                        </div>
                    @endif
                ...
                <div x-show="activeTab === 'pending'" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" x-cloak>
                            ...
                            <table class="min-w-full divide-y divide-slate-200 dark:divide-slate-600">
                                <thead>
                                    <tr class="bg-slate-50 dark:bg-slate-700/50">
                                        ...
                                        <th scope="col" class="px-5 py-3 text-left text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">
                                            {{ __('scientific.invitation_sent') }}
                                        </th>
                                        <th scope="col" class="px-5 py-3 text-left text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">
                                            {{ __('scientific.expires') }}
                                        </th>
                                    </tr>
                                </thead>
                                <tbody class="bg-white dark:bg-slate-800 divide-y divide-slate-200 dark:divide-slate-600">
                    ...
                    @else
                        ...
                        <div class="text-center py-12">
                            ...
                            <h3 class="text-base font-semibold text-slate-900 dark:text-white mb-1">{{ __('scientific.no_pending_invitations') }}</h3>
                            <p class="text-sm text-slate-500 dark:text-slate-400">{{ __('scientific.no_pending_invitations_desc') }}</p>
                        </div>
                    @endif
                </div>
            </div>
                    ...
                    <div class="bg-slate-50 dark:bg-slate-800/50 px-6 py-4 border-b border-slate-200 dark:border-slate-700 rounded-t-xl">
                        <div class="flex items-center justify-between">
                            <h3 class="text-lg font-semibold text-slate-900 dark:text-white">
                                {{ __('scientific.invite_new_instructors') }}
                            </h3>
                            ...
                            <div class="flex">
                                ...
                                <div class="ml-3">
                                    <p class="text-sm text-blue-700 dark:text-blue-300">
                                        <strong>{{ __('scientific.conditions') }}:</strong>
                                        {{ __('scientific.condition_active_license') }}
                                        {{ __('scientific.condition_active_certification') }}
                                    </p>
                                </div>
                            </div>
                        </div>

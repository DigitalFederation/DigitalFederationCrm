...
class ActivateAfterPaymentCertificationAttributedListener
{
        ...
        foreach ($event->models as $modelArray) {
            foreach ($modelArray as $model) {
                if ($model instanceof CertificationAttributed) {
                    ...
                    if ($model->certification->isInternationalCertification() && ! $model->cmas_international_code) {
                        $model->cmas_international_code = $this->generateInternationalCode($model);
                    }
                    ...
                    Log::info('ActivateAfterPaymentCertificationAttributedListener: Certification activated after payment', [
                        ...
                        'cmas_international_code' => $model->cmas_international_code,
                    ]);
    ...
    /**
     * Generate CMAS international certification code
     */
    private function generateInternationalCode(CertificationAttributed $certificationAttributed): string
    {
        $year = now()->year;

        // Get the last CMAS code for this year
        $lastCertification = CertificationAttributed::whereYear('created_at', $year)
            ->whereNotNull('cmas_international_code')
            ->orderBy('id', 'desc')
            ->first();

        $sequentialNumber = 1;
        if ($lastCertification && preg_match('/\d+$/', $lastCertification->cmas_international_code, $matches)) {
            $sequentialNumber = intval($matches[0]) + 1;
        }

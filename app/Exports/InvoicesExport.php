...
class InvoicesExport implements FromCollection, WithHeadings, WithMapping
{
    ...
    public function map($detail): array
    {
        ...
        [$ownerType, $ownerName, $ownerCmasCode, $federationName] = $this->getOwnerInformation($document);
    ...
    protected function getOwnerInformation(Document $document): array
    {
        ...
        $type = $name = $cmasCode = $federationName = '';

        if ($owner) {
            if ($owner instanceof Federation) {
                $type = 'Federation';
                $name = $owner->name;
                $cmasCode = $owner->code_cmas ?? '';
        ...
        return [$type, $name, $cmasCode, $federationName];
    }

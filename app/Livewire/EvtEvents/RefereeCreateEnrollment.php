...
class RefereeCreateEnrollment extends Component implements HasForms, HasTable
{
    ...
    public array $refereeAttributes = [];
    ...
    public function mount(Event $event): void
    {
        ...
        if ($this->isAdminUser()) {
            ...
            if (! $this->federation) {
                abort(403, __('events.not_authorized_referee_enrollment'));
            }
        }

        $this->loadRefereeAttributes();
    }

    protected function loadRefereeAttributes(): void
    {
        // Get the event's selected referee attributes
        $eventRefereeAttributes = $this->event->refereeAttributes()->get();

        // If the event has no referee attributes selected, fall back to the default behavior
        if ($eventRefereeAttributes->isEmpty()) {
            $attributesAction = new GetAttributesAndRulesFromRolesAction;
            $attributes = $attributesAction->execute(EvtEventEnrollmentRoleEnum::TECHNICAL_OFFICIAL->value);
        } else {
            // Use the event's selected referee attributes
            $attributes = $eventRefereeAttributes->map(function ($attribute) {
                return [
        ...
        $this->refereeAttributes = collect($attributes)->map(function ($attribute) {
    ...
    private function initializeAttributeValues(): void
    {
        foreach ($this->selectedIndividuals as $individual) {
            ...
            foreach ($this->refereeAttributes as $attribute) {
    ...
    {
        return (new GetEligibleRefereesAction)->execute(
            $this->event,
            $this->federation->id,
            'federation'
        );
        ...
        try {
            foreach ($this->selectedIndividuals as $selectedIndividual) {
                ...
                (new CreateRefereeEnrollmentAction)->execute(
                    event: $this->event,
                    enrollable: $this->federation,
                    individualId: (string) $individual->getKey(),
            ...
            $redirectRoute = match (true) {
                $this->isAdminUser() => 'admin.evt-events.events.referee-enrollment.index',
                $this->isEntityUser() => 'entity.evt-events.events.referee-enrollment.index',
                default => 'federation.evt-events.events.referee-enrollment.index',
            };

            return redirect()->route($redirectRoute, [
                'event' => $this->event->getKey(),
            ])->with('success', __('events.referees_enrolled_successfully'));
    ...
    {
        return view('livewire.evt-events.referee-create-enrollment', [
            'event' => $this->event,
            'enrollmentType' => 'referee',
        ]);
    }
}

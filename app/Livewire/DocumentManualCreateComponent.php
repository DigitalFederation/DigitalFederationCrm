...
class DocumentManualCreateComponent extends Component
{
    ...
    {
        // Validate if either a product is selected or a description is given, but not both
        if ($this->selectedProductId && ! empty($this->newDetail['description'])) {
            // Both are filled, return an error
            $this->errorMessage = 'Please choose either a product or manually enter a description, not both.';

            return;
        } elseif (! $this->selectedProductId && empty($this->newDetail['description'])) {
            // Neither is filled, return an error
            $this->errorMessage = 'Please choose either a product or manually enter a description, not both.';
        ...
        if ($this->selectedProductId) {
            ...
            if ($product) {
                // Populate the detail fields with product data
                $this->newDetail = [
                    'owner_id' => $product->id,

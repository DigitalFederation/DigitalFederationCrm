...
class GetIndividualAndInstructorForCertification extends Component
{
    ...
    private function handleFederationApproval(): void
    {
        if ($this->federationApprove) {
            // For CMAS admin, use all certifications passed from controller
            if ($this->is_cmas && ! empty($this->passedCertifications)) {
    ...
    public function searchIndividual(): void
    {
        ...
        $individualSearch = Individual::where('code_cmas', $this->codeIndividual)
            ->individualsFromEntity()
            ->first(['id', 'code_cmas', 'name', 'surname']);
    ...
    {
        $query = Individual::where('code_cmas', $this->codeInstructor)
    ...
    public function searchAssistantInstructor(DetectIfIndividualIsInstructorAction $checkInstructor): void
    {
        ...
        $this->assistantInstructor[] = $assistant->only(['code_cmas', 'name', 'surname', 'id']);
    ...
    {
        return $this->instructor && $this->codeAssistantInstructor == $this->instructor->code_cmas;
    }

    private function buildAssistantInstructorQuery(): Builder
    {
        return Individual::where('code_cmas', $this->codeAssistantInstructor)
            ->individualsFromEntity()
            ->with('licenses');
    }

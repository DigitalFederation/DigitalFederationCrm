...
use Domain\Entities\Actions\SyncEntityFeaturedLocationsAction;
...
class ManagePublicPage extends Component
{
    ...
    public array $selectedFeaturedLocations = [];

    public array $availableDivingLocations = [];

    protected $listeners = [
        'selectedMultipleUpdatedValue.featured-locations' => 'updateSelectedLocations',
    ];
    ...
    public function mount(): void
    {
        ...
        $this->selectedFeaturedLocations = $entity->featuredDivingLocations()->pluck('diving_location.id')->toArray();

        $this->loadAvailableDivingLocations();
    }

    protected function loadAvailableDivingLocations(): void
    {
        $this->availableDivingLocations = DivingLocation::query()
            ->orderBy('name')
            ->get(['id', 'name', 'region'])
            ->mapWithKeys(fn (DivingLocation $location) => [
    ...
    public function updateSelectedLocations(array $values): void
    {
        $this->selectedFeaturedLocations = array_map('intval', $values);
    }
    ...
    public function saveFeaturedLocations(SyncEntityFeaturedLocationsAction $action): void
    {
        $this->authorizeEntityAccess();

        $action($this->entity, $this->selectedFeaturedLocations);

        Notification::make()
            ->title(__('entity.public_page.featured_locations.locations_saved'))
            ->success()
            ->send();
    }

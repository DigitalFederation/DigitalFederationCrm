...
class LicensePurchaseForm extends Component
{
    ...
    public function updatedSelectedLicenseId($licenseId)
    {
        ...
        if ($licenseId) {
            ...
            } elseif (strtoupper($this->committee) === 'SPORT') {
                // Sport committee - only national licenses
                $query->whereHas('committee', fn ($q) => $q->where('is_international', false));
            } elseif ($this->committee && strtoupper($this->committee) !== 'SPORT') {
                // Other committees (like diving) - only international licenses
                $query->withoutGlobalScope(ExcludeInternationalScope::class)
                    ->whereHas('committee', fn ($q) => $q->where('is_international', true));
            } else {
    ...
    private function loadLicenses()
    {
        ...
        if ($this->isInternational !== null) {
            ...
            // SPORT committee can only see national licenses
            $query->whereHas('committee', fn ($q) => $q->where('is_international', false));
        } elseif ($this->committee && strtoupper($this->committee) !== 'SPORT') {
            // Other specific committees (like diving, scientific) can only see international licenses
            $query->withoutGlobalScope(ExcludeInternationalScope::class)
                ->whereHas('committee', fn ($q) => $q->where('is_international', true));
        } else {

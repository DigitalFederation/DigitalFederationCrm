...
class EntityController extends Controller
{
    ...
    public function index(): View
    {
        $entities = QueryBuilder::for(Entity::class)
            ->allowedFilters([
                ...
                AllowedFilter::scope('filter_cmas_code'),
                ...
                AllowedFilter::exact('has_cmas_portal'),
            ])
    ...
    public function show(string $id): View
    {
        ...
        // Detect if user is a CMAS admin
        $user = Auth::user();
        $cmasAdminRoles = [
            'admin',
        ...
        $isCmasAdmin = $user && $user->hasAnyRole($cmasAdminRoles);

        // Detect if entity is related to the CMAS federation
        $cmasFederation = $entity->federations()->where('is_default_federation', true)->first();
        $isCmasEntity = $cmasFederation !== null;

        $showCmasCompanyView = $isCmasAdmin && $isCmasEntity;

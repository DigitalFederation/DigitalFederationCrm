...
class MembershipPlanController extends Controller
{
    ...
    public function index(): View
    {
        ...
        // Manually insert the "CMAS" option
        $committees->prepend(['id' => null, 'name' => 'CMAS']);
    ...
    public function create(): View
    {
        ...
        $products = Product::select('id', 'name')->orderBy('name')->pluck('name', 'id');

        return view('web.admin.membership_plan.create', compact('committees', 'interval_unit', 'licenses', 'products'));
    }
    ...
    {
        $plan = MembershipPlan::with('licenses', 'products')->find($id);
        $committees = Committee::select('id', 'name')->orderBy('name')->get();
        $interval_unit = config('enum.interval_unit');
        $licenses = License::select('id', 'name')->orderBy('name')->pluck('name', 'id');
        $products = Product::select('id', 'name')->orderBy('name')->pluck('name', 'id');

        return view('web.admin.membership_plan.edit', compact('plan', 'committees', 'interval_unit', 'licenses', 'products'));
    }

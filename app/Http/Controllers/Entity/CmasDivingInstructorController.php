...
class CmasDivingInstructorController extends Controller
{
    /**
    ...
    public function index(): View
    {
        ...
        // Fetch Professional Roles for Livewire component (CMAS Diving - INSTRUCTOR and LEADER roles)
        ...
        return view('web.entity.cmas_diving_instructor.index', compact(
            'entity',
            'instructors',
            'professionalRoles',
            'pendingInvitations'
    ...
    public function cancelInvitation($invitationId): RedirectResponse
    {
        ...
        if ($invitation->status_class !== \Domain\Entities\States\PendingEntityProfessionalRoleState::class) {
            return back()->with('error', __('cmas_diving.cannot_cancel_invitation'));
        }
        ...
        try {
            ...
            return redirect()->route('entity.cmas-diving-instructor.index')
                ->with('success', __('cmas_diving.invitation_canceled_successfully'));

        } catch (\Exception $e) {
            Log::error('Failed to cancel invitation: ' . $e->getMessage());

            return back()->with('error', __('cmas_diving.failed_to_cancel_invitation'));
        }
    }
    ...
    public function remove(Request $request, $professionalId): RedirectResponse
    {
        ...
        try {
            if ($validated['action'] === 'deactivate') {
                ...
                return redirect()->route('entity.cmas-diving-instructor.index')
                    ->with('success', __('cmas_diving.professional_deactivated_successfully'));
            } else {
                $professional->delete();

                return redirect()->route('entity.cmas-diving-instructor.index')
                    ->with('success', __('cmas_diving.professional_removed_successfully'));
            }

        } catch (\Exception $e) {
            Log::error('Failed to remove/deactivate professional: ' . $e->getMessage());

            return back()->with('error', __('cmas_diving.failed_to_remove_professional'));
        }
    }
        ...
        try {
            ...
            activity()
                ...
                ->withProperties([
                    ...
                    'cmas_code' => $entityProfessionalRole->individual->code_cmas,
                ])
                ->log("CMAS diving instructor relationship with CMAS code {$entityProfessionalRole->individual->code_cmas} removed");

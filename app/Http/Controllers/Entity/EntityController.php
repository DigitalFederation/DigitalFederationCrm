...
class EntityController extends Controller
{
    ...
    public function edit(): View|RedirectResponse
    {
        ...
        // Only fetch diving locations if user has diving admin role
        $divingLocations = [];
        $featuredLocationIds = [];
        if (Auth::user()->hasRole('entity-diving-services')) {
            ...
            $featuredLocationIds = $entity->featuredDivingLocations->pluck('id')->toArray();
        }
    ...
    public function update(
        ...
        $validated = $request->validate([
            ...
            'featured_diving_locations' => 'nullable|array',
            'featured_diving_locations.*' => 'exists:diving_location,id',
        ]);
        ...
        // Only allow diving locations to be updated by diving admin
        if (! Auth::user()->hasRole('entity-diving-services')) {
            unset($validated['featured_diving_locations']);
        }
        ...
        try {
            ...
            // Sync featured diving locations only if user has diving admin role
            if (Auth::user()->hasRole('entity-diving-services')) {
                $featuredIds = $request->input('featured_diving_locations', []); // Default to empty array if not present
                $entity->featuredDivingLocations()->sync($featuredIds);
            }
        } catch (Exception $ex) {

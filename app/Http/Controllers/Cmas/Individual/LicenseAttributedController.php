...
/**
 * CMAS Individual License Attributed Controller
 *
...
class LicenseAttributedController extends BaseLicenseAttributedController
{
    ...
    public function index(): View
    {
        ...
        if (! $individual->hasActiveAffiliation()) {
            abort(403, __('cmas.error.no_active_affiliation'));
        }
        ...
        $title = __('cmas.licenses.title');

        return view('web.cmas.individual.licenses-attributed.index', compact(
            'licenses',
            'federations',
            'countries',
            'sports',
    ...
    public function show(string $id): View|RedirectResponse
    {
        ...
        return view('web.cmas.individual.licenses-attributed.show', compact('license'));
    }
    ...
    public function destroy(LicenseAttributed $license_attributed): RedirectResponse
    {
        ...
        if (! $license_attributed->canBeDeleted()) {
            return redirect()->back()->with('error', __('cmas.error.cannot_delete_license'));
        }

        $license_attributed->delete();

        return redirect()->route('cmas.individual.licenses-attributed.index')
            ->with('success', __('cmas.success.license_deleted'));
    }
}

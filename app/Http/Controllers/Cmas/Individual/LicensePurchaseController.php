...
namespace App\Http\Controllers\Cmas\Individual;
...
/**
 * CMAS Individual License Purchase Controller
 *
...
class LicensePurchaseController extends Controller
{
    ...
    public function index(Request $request): View
    {
        ...
        if (! $individual) {
            abort(403, __('cmas.error.no_individual_profile'));
        }
        ...
        // Validate and sanitize committee filter - only allow CMAS committees
        ...
        return view('web.cmas.individual.license-purchase.index', [
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        if (! $individual || $individual->user_id !== Auth::id()) {
            abort(403, __('cmas.error.unauthorized'));
        }
        ...
        if (! $license) {
            ...
            if ($nationalLicense) {
                return back()
                    ->withErrors(['license_id' => __('cmas.error.not_international_license')])
                    ->withInput();
            }

            return back()
                ->withErrors(['license_id' => __('cmas.error.license_not_found')])
                ->withInput();
        }
        ...
        try {
            ...
            return redirect()
                ->route('cmas.individual.license-purchase.success')
                ->with('success', __('cmas.success.license_purchase_initiated'))
    ...
    public function success(): View|RedirectResponse
    {
        ...
        if (! $licenseAttributedId) {
            return redirect()->route('cmas.individual.license-purchase.index')
                ->with('error', __('cmas.error.no_license_purchase'));
        }
        ...
        if (! $licenseAttributed) {
            return redirect()->route('cmas.individual.license-purchase.index')
                ->with('error', __('cmas.error.license_purchase_not_found'));
        }
        ...
        return view('web.cmas.individual.license-purchase.success', compact(
            'licenseAttributed',
            'license',
            'document',
            'individual'

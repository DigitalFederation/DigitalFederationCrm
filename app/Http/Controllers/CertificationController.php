    ...
    public function search(PublicSearchCertificationRequest $request): View
    {
        $getCertifications = CertificationAttributed::select('id', 'certification_name', 'certification_id', 'federation_id', 'created_at', 'status_class', 'current_term_starts_at', 'national_code')
            ->whereHas('individual', function (Builder $query) use ($request) {
                ...
                $query->select('id', 'name', 'surname', 'birthdate', 'code_cmas')
                    ...
                    })->when(! empty($validated['code_cmas']), function (Builder $query) use ($validated) {
                        $query->where('code_cmas', $validated['code_cmas']);
                    });
            })
        ...
        // Get Individual from the request code_cmas
        $individual = null;
        $validatedData = $request->validated(); // Get validated data once

        if (! empty($validatedData['code_cmas'])) {
            $individual = Individual::where('code_cmas', $validatedData['code_cmas'])->first();
        ...
        $certificationsByCommittee = CertificationAttributed::query()
            ->when($request->filled('code_cmas'), fn($q) =>
            $q->whereHas('individual', fn($q) => $q->where('code_cmas', $request->get('code_cmas'))))

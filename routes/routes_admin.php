...
use App\Http\Controllers\Admin\CertificationController as CmasCertificationController;
use App\Http\Controllers\Admin\CertificationRoleMappingController;
use App\Http\Controllers\Admin\DashboardController as CmasDashboardController;
use App\Http\Controllers\Admin\DistrictController;
use App\Http\Controllers\Admin\DivingCourseController as CmasDivingCourseController;
/* ------------------------------------------------------------------------
 |  Controller imports – kept together for easy scanning
 |  (⌘/Ctrl-F on the filename to jump straight here)
 * --------------------------------------------------------------------- */
use App\Http\Controllers\Admin\DivingLocationController as CmasDivingLocationController;
use App\Http\Controllers\Admin\DocumentController as CmasDocumentController;
use App\Http\Controllers\Admin\DocumentManualPaymentController;
use App\Http\Controllers\Admin\EntityController as CmasEntityController;
...
use App\Http\Controllers\Admin\FederationController as CmasFederationController;
use App\Http\Controllers\Admin\FederationRoleMappingController;
use App\Http\Controllers\Admin\FederationVotingRightController as CmasFederationVotingRightController;
use App\Http\Controllers\Admin\GeneratedReportsController;
use App\Http\Controllers\Admin\ImpersonateController;
use App\Http\Controllers\Admin\IndividualController as CmasIndividualController;
...
use App\Http\Controllers\Admin\LicenseAttributedController as CmasLicensesAttributedController;
use App\Http\Controllers\Admin\LicenseController as CmasLicenseController;
...
use App\Http\Controllers\Admin\ProductsController as CmasProductsController;
...
use App\Http\Controllers\Admin\ShippingPricesController;
use App\Http\Controllers\Admin\ShippingSubZoneController;
use App\Http\Controllers\Admin\ShippingWeightController;
...
use App\Http\Controllers\Admin\UsersController as CmasUsersController;
...
Route::prefix('admin')
    ...
    ->group(function () {
        ...
        // Dashboard
        Route::get('dashboard', [CmasDashboardController::class, 'index'])
            ->name('dashboard');
        ...
        Route::middleware('permission:access memberships')->group(function () {
            ...
            Route::prefix('insurances/{insurance}/document')->group(function () {
                ...
                // TEMPORARY: Preview route for PDF development
                Route::get('preview', function ($insurance) {
                    $insuranceModel = \Domain\Insurance\Models\Insurance::with(['member', 'insurancePlan', 'memberSubscription.membershipPackage', 'requester'])
                        ->findOrFail($insurance);

                    if ($insuranceModel->member_type !== \Domain\Individuals\Models\Individual::class) {
                        abort(404, 'Insurance document is only applicable to Individual members.');
                    }

                    $individual = $insuranceModel->member;
                    $insurancePlan = $insuranceModel->insurancePlan;
                    $policyNumber = $insuranceModel->policy_number ?? $insurancePlan->policy_number;
                    ...
                    return view('shared.insurance.document-pdf', [
                        ...
                        'startDateFormatted' => $insuranceModel->start_date->format('d/m/Y'),
                        'endDateFormatted' => $insuranceModel->end_date->format('d/m/Y'),
                    ]);
                })->name('insurances.document.preview');
            });
        });
        ...
        Route::middleware('permission:access users')->group(function () {
            ...
            // Users
            Route::controller(CmasUsersController::class)->group(function () {
        ...
        Route::middleware('permission:access federations')->group(function () {
            ...
            // Federations
            Route::controller(CmasFederationController::class)->group(function () {
            ...
            // Voting rights
            Route::controller(CmasFederationVotingRightController::class)->group(function () {
        ...
        Route::middleware('permission:access entities')->group(function () {
            ...
            Route::resource('entities', CmasEntityController::class)
                ->except(['show'])
                ->names('entity');
            Route::get('entity/{entity}', [CmasEntityController::class, 'show'])
                ->name('entity.show');
            Route::post('entity/{entity}/send-welcome-email', [CmasEntityController::class, 'sendWelcomeEmail'])
        ...
        Route::middleware('permission:access individuals')->group(function () {
            ...
            Route::get('individual', [CmasIndividualController::class, 'index'])->name('individual.index');
            Route::get('individual/create', [CmasIndividualController::class, 'create'])->name('individual.create');
            Route::post('individual', [CmasIndividualController::class, 'store'])->name('individual.store');
            Route::get('individual/{individual}', [CmasIndividualController::class, 'show'])->name('individual.show');
            Route::get('individual/{individual}/edit', [CmasIndividualController::class, 'edit'])->name('individual.edit');
            Route::put('individual/{individual}', [CmasIndividualController::class, 'update'])->name('individual.update');
            Route::patch('individual/{individual}', [CmasIndividualController::class, 'update']);
            Route::delete('individual/{individual}', [CmasIndividualController::class, 'destroy'])->name('individual.destroy');
            Route::get('individual/{individual}/files', [CmasIndividualController::class, 'files'])->name('individual.files');
            Route::get('individual/{individual}/update-email', [CmasIndividualController::class, 'showUpdateEmailForm'])->name('individual.show-update-email');
            Route::put('individual/{individual}/update-email', [CmasIndividualController::class, 'updateEmail'])->name('individual.update-email');
            Route::post('individual/{individual}/send-welcome-email', [CmasIndividualController::class, 'sendWelcomeEmail'])
        ...
        Route::middleware('permission:access certifications')->group(function () {
            ...
            Route::get('certifications/diving', [CmasCertificationController::class, 'indexDiving'])
                ->name('certification.diving');
            Route::get('certifications/sport', [CmasCertificationController::class, 'indexSport'])
                ->name('certification.sport');

            // main CRUD
            Route::resource('certifications', CmasCertificationController::class)
                ->names('certification')
                ->except(['show']);
        ...
        Route::middleware('permission:access licenses')->group(function () {
            Route::resource('licenses', CmasLicenseController::class)
                ->names('license')
            ...
            // Attributed licenses
            Route::controller(CmasLicensesAttributedController::class)
            ...
            Route::controller(AdminSeparatedLicenseAttributedController::class)->group(function () {
                // Sport (National)
                Route::get('/sport-entity-licenses-attributed', 'sportEntity')
                    ->name('sport-entity-licenses-attributed.index');
                Route::get('/sport-individual-licenses-attributed', 'sportIndividual')
                    ->name('sport-individual-licenses-attributed.index');

                // National diving services
                Route::get('/national-diving-entity-licenses-attributed', 'nationalDivingEntity')
                    ->name('national-diving-entity-licenses-attributed.index');
                Route::get('/national-diving-individual-licenses-attributed', 'nationalDivingIndividual')
                    ->name('national-diving-individual-licenses-attributed.index');

                // International diving
                Route::get('/cmas-diving-entity-licenses-attributed', 'divingCmasEntity')
                    ->name('cmas-diving-entity-licenses-attributed.index');
                Route::get('/cmas-diving-individual-licenses-attributed', 'divingCmasIndividual')
                    ->name('cmas-diving-individual-licenses-attributed.index');

                // Scientific (International)
                Route::get('/scientific-entity-licenses-attributed', 'scientificEntity')
                    ->name('scientific-entity-licenses-attributed.index');
                Route::get('/scientific-individual-licenses-attributed', 'scientificIndividual')
                    ->name('scientific-individual-licenses-attributed.index');
            });
        });
        ...
        Route::middleware('permission:access documents')->group(function () {
            Route::controller(CmasDocumentController::class)->group(function () {
        ...
        /* -------------------------------------------------------------------- */
        /*  Diving locations & courses */
        /* -------------------------------------------------------------------- */
        Route::middleware('permission:access diving location')->group(function () {
            Route::resource('diving-location', CmasDivingLocationController::class)
                ->names('diving-location');
        });
        Route::resource('diving-course', CmasDivingCourseController::class)
            ->names('diving-course')
        ...
        /*  Products */
        /* -------------------------------------------------------------------- */
        Route::middleware('permission:access products')
            ->controller(CmasProductsController::class)
            ->prefix('products')
            ->name('products.')
            ->group(function () {
                Route::get('/', 'index')->name('index');
        ...
        Route::prefix('shipping')
            ...
            ->group(function () {
                ...
                Route::resource('sub-zones', ShippingSubZoneController::class);
                Route::resource('methods', ShippingMethodController::class)
                    ->except(['show']); // Controller has no show() method
                Route::resource('weights', ShippingWeightController::class)
                    ->except(['show']); // Controller has no show() method
                Route::resource('prices', ShippingPricesController::class);
            });

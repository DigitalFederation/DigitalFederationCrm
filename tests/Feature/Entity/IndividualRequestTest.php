...
beforeEach(function () {
    ...
    $this->individual = Individual::factory()->create(['code_cmas' => '123456']);
...
it('action directly creates relationship', function () {
    ...
    expect($this->federation->entities)->toHaveCount(1)
        ...
        ->and($this->individual->code_cmas)->toBe('123456');
...
it('creates an invitation for an eligible individual', function () {
    ...
    $component = Livewire::actingAs($this->user)
        ...
        ->set('code_cmas', '123456');

    $component->call('submit')
        ->assertHasNoErrors(['code_cmas']);
...
it('shows an error for invalid CMAS code', function () {
    Livewire::actingAs($this->user)
        ->test(IndividualRequest::class)
        ->set('code_cmas', '999999')
        ->call('submit')
        ->assertHasErrors(['code_cmas' => 'exists']);
});
...
it('requires at least one identifier to be filled', function () {
    Livewire::actingAs($this->user)
        ...
        ->set('member_number', '')
        ->call('submit')
        ->assertHasErrors(['code_cmas' => 'required_without', 'member_number' => 'required_without']);
});

...
test('SyncEntityFeaturedLocationsAction syncs locations correctly', function () {
    actingAs($this->entityUser);

    $locations = DivingLocation::factory()->count(3)->create();
    $locationIds = $locations->pluck('id')->toArray();

    $action = new SyncEntityFeaturedLocationsAction;
    $action($this->entity, $locationIds);

    expect($this->entity->featuredDivingLocations()->count())->toBe(3);
    expect($this->entity->featuredDivingLocations()->pluck('diving_location.id')->toArray())
        ->toEqual($locationIds);
});

test('SyncEntityFeaturedLocationsAction removes locations when empty array provided', function () {
    actingAs($this->entityUser);

    $locations = DivingLocation::factory()->count(2)->create();
    $this->entity->featuredDivingLocations()->sync($locations->pluck('id')->toArray());

    $action = new SyncEntityFeaturedLocationsAction;
    $action($this->entity, []);

    expect($this->entity->featuredDivingLocations()->count())->toBe(0);
});

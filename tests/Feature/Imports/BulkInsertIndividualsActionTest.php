...
test('associates individuals with multiple federations', function () {
    ...
    // Check that each individual has a unique code_cmas
    expect($john->code_cmas)->not->toBeNull()
        ->and($jane->code_cmas)->not->toBeNull()
        ->and($john->code_cmas)->not->toBe($jane->code_cmas);
});

...
beforeEach(function () {
    ...
    $this->internationalCertification = Certification::factory()->create([
        'committee_id' => $this->divingCommittee->id,
        'name' => 'CMAS Diving Certification',
    ]);
});
...
test('entity can only see its own international licenses', function () {
    ...
    $response = $this->actingAs($user1)->get(route('cmas.entity.licenses-attributed.index'));
    $response->assertSuccessful();

    // Entity 2 should only see their own license
    $response = $this->actingAs($user2)->get(route('cmas.entity.licenses-attributed.index'));
    $response->assertSuccessful();
});
...
test('entity can see international certifications for their members only', function () {
    ...
    $response = $this->actingAs($user)->get(route('cmas.entity.certifications.index'));
    $response->assertSuccessful();
...
test('entity cannot access licenses of other entities', function () {
    ...
    $response = $this->actingAs($user1)->get(route('cmas.entity.licenses-attributed.show', $license2->id));
    $response->assertNotFound();
});

test('entity requires permission to access cmas routes', function () {
    $user = User::factory()->create(['group_id' => $this->entityGroup->id]);
    $entity = Entity::factory()->create();
    $entity->users()->attach($user->id);
    // No permission given
    ...
    // Should be forbidden
    $response = $this->actingAs($user)->get(route('cmas.entity.licenses-attributed.index'));
    $response->assertForbidden();

    $response = $this->actingAs($user)->get(route('cmas.entity.license-purchase.index'));
    $response->assertForbidden();
...
test('entity with permission can access cmas routes', function () {
    $user = User::factory()->create(['group_id' => $this->entityGroup->id]);
    $entity = Entity::factory()->create();
    $entity->users()->attach($user->id);
    $user->givePermissionTo('access international licenses');

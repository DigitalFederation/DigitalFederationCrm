...
it('can delete a license attributed as a CMAS user', function () {
    // Create a CMAS user
    $cmasUser = User::factory()->create([
        'group_id' => Group::where('code', 'ADMIN')->first()->id,
    ]);
    $cmasUser->assignRole('admin');
    ...
    // Attempt to delete the LicenseAttributed as CMAS user
    actingAs($cmasUser);
...
it('cannot delete a license attributed as a non-CMAS user', function () {
    // Create a non-CMAS user (e.g., INDIVIDUAL)
    $nonCmasUser = User::factory()->create([
    ...
    // Attempt to delete the LicenseAttributed as non-CMAS user
    actingAs($nonCmasUser);
    $response = delete(route('admin.license-attributed.delete', $licenseAttributed->id));

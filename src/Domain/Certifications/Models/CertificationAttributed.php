...
class CertificationAttributed extends Model
{
    ...
    {
        $code = $this->federation?->code_cmas ?? '';
    ...
    public function scopeDirectorCode(Builder $query, string $cmas_code): Builder
    {
        return $query->whereHas('mainInstructor', function (Builder $query) use ($cmas_code) {
            return $query->where('code_cmas', 'like', '%' . $cmas_code . '%');
        });
    }
    ...
    public function scopeCmasCode(Builder $query, string $code_cmas): Builder
    {
        return $query->whereHas('individual', function (Builder $query) use ($code_cmas) {
    ...
    /**
     * Scope a query to filter by the related individual's CMAS code.
     */
    public function scopeFilterIndividualCmasCode(Builder $query, string $code): Builder
    {
        return $query->whereHas('individual', function (Builder $q) use ($code) {
            $q->where('code_cmas', 'like', '%' . $code . '%');
        });
    }

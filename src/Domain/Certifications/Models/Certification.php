...
class Certification extends Model
{
    ...
    /**
     * Check if certification requires CMAS operator role for entities
     */
    public function requiresCmasOperatorRole(): bool
    {
        // International certifications require CMAS operator role
        return $this->isInternationalCertification();
    }

    /**
     * Check if this certification's committee is an international committee.
     * International committees: DIVING, SCIENTIFIC (CMAS committees)
     */
    ...
    {
        // If entity doesn't have CMAS operator role, exclude international certifications
        if (! $entity->hasRole('entity-cmas')) {
            return $query->whereHas('committee', function (Builder $q) {
                $q->where('is_international', false);
            });
        }

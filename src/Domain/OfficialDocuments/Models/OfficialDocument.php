...
class OfficialDocument extends Model implements HasMedia
{
    ...
    public function scopeFilterCmasCode(Builder $query, $cmasCode): Builder
    {
        return $query->whereHas('individual', function ($q) use ($cmasCode) {
            $q->where('code_cmas', 'like', '%' . $cmasCode . '%');
        });
    }

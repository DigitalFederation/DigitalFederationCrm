...
class Document extends Model
{
    ...
    /**
     * Scope a query to only include documents whose owner (Federation, Entity, or Individual) has the given CMAS code.
     */
    public function scopeFilterCmasCode(Builder $query, string $cmasCode): Builder
    {
        return $query->where(function ($q) use ($cmasCode) {
            $q->whereHasMorph(
                'owner',
                [
                ...
                function (Builder $ownerQuery) use ($cmasCode) {
                    $ownerQuery->where('code_cmas', $cmasCode);
                }
            );
        });
    }

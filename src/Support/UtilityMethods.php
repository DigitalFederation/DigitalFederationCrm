...
class UtilityMethods
{
    ...
    public static function generateUniqueIndividualCode(): string
    {
        ...
        if (Individual::where('code_cmas', $random_str)->count() > 0) {
    ...
    {
        $random_str = Str::random(7);
        if (Entity::where('code_cmas', $random_str)->count() > 0) {
    ...
    {
        $random_str = Str::random(7);
        if (Federation::where('code_cmas', $random_str)->count() > 0) {
    ...
    public static function generateCertificationCmasInternationalNumber(int $year, int $federationId): string
    {
        ...
        return $year . $federation->code_cmas . $uniqueString;
    }
